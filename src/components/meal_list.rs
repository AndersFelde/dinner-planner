...
#[server]
pub async fn get_all_meals_with_ingredients(
) -> Result<Vec<MealWithIngredients>, ServerFnError> {
    use crate::db::*;
                    ...
                    view! {
                        <li class="mb-6">
                            <div class="p-4 rounded-lg shadow bg-white dark:bg-gray-900">
                                <h2 class="text-lg font-bold text-gray-800 dark:text-gray-100 mb-2">{meal.meal.name.clone()}</h2>
                                <div class="mb-2 text-gray-700 dark:text-gray-200 font-semibold">Ingredients:</div>
                                <ul class="flex flex-wrap gap-2">
                                    {meal.ingredients.iter().map(|ingredient| view! {
                                                <li>
                                                    <button
                                                        type="button"
                                                        class=format!(
